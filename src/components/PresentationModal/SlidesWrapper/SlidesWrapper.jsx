...
const SlidesWrapper = ({ slides, currentSlide, isFullscreen, onNextSlide, title, entity, year }) => {
  ...
  const renderTitleSlide = () => (
    <div className={c.titleSlide}>
      <h1>{title}</h1>
      {(entity || year) && <p>{[entity, year].filter(Boolean).join(" • ")}</p>}
    </div>
  );
  ...
  const getCurrentSlideContent = () => {
    ...
    return renderSlideContent(slides[currentSlide - 1]);
  };
